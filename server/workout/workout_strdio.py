...
def workout_strdio(weeks):
    set1 = "total 30 mins for each, take break between 10 mins"
    set2 = "Lift the weights suited for you, Totally 3 sets, Each Set 12 reps, take rest between each sets "
    ...
    for i in range(n):
        ...
        def generate_random_workout(user_preferences):
            ...
            workouts = ['strdio_Training_1','strdio_Training_2','strdio_Training_3','strdio_Training_4']
            workout = random.choice(workouts)
            
            if workout == 'strdio_Training_1':
                strdio_Training_1_exercises = ['Walking, Jogging(Mixed)','walking','jogging']
                workout_plan = random.sample(strdio_Training_1_exercises, k=1)  
           
            elif workout == 'strdio_Training_2':
                strdio_Training_2_exercises = ['Butt Kicks, High Knees, toe taps','Squat Jumps, Toe Taps', 'Standing alternative Toe touches' ]
                workout_plan = random.sample(strdio_Training_2_exercises, k=1) 
           
            elif workout == 'strdio_Training_3':
                strdio_Training_3_excercises = ['Lat pulldown,Suspended row','Quadruped single-arm dumbbell row,chin Ups']
                workout_plan = random.sample(strdio_Training_3_excercises, k=1)  
            elif workout == 'strdio_Training_4':
                strdio_Training_4_excercises = ['Tense and Hold Press-Ups, Wide Grip Press-Up, T Press-Up','Spiderman Press-Up,Normal Press-Up', 'Decline Press-Up, Inclined Press-Up' ]
                workout_plan = random.sample(strdio_Training_4_excercises, k=1)  
            
            else:
                cardio_Training_4_excercises = ['Bottle Slider Flyes,Kitchen Sieve Close-Grip Press-Up,The Alternating Shuffle Press-up', 'Tricep PushUP,Tricep dips']
                workout_plan = random.sample(cardio_Training_4_excercises, k=1)  
            return workout_plan
