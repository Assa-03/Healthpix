...
def workout_cardio(weeks):
    n = weeks
    set = "total 30 mins for each, take break between 10 mins"
    work = {}
    out =[]
    final = []
    ...
    for i in range(n):
        ...
        def generate_random_workout(user_preferences):
            ...
            workouts = ['cardio_Training_1','cardio_Training_2','cardio_Training_3']
            workout = random.choice(workouts)
            
            if workout == 'cardio_Training_1':
                cardio_Training_1_exercises = ['Walking, Jogging(Mixed)','walking','jogging']
                workout_plan = random.sample(cardio_Training_1_exercises, k=1)  
           
            elif workout == 'cardio_Training_2':
                cardio_Training_2_exercises = ['Butt Kicks, High Knees, toe taps','Squat Jumps, Toe Taps', 'Standing alternative Toe touches' ]
                workout_plan = random.sample(cardio_Training_2_exercises, k=1) 
           
            elif workout == 'cardio_Training_3':
                cardio_Training_3_excercises = ['Bicycle Crunches,Mountain Climbers,Leg Raises,Reverse Crunch','Normal Squates, Sumo Squates, Lunges']
                workout_plan = random.sample(cardio_Training_3_excercises, k=1)  
            
            else:
                cardio_Training_4_excercises = ['box jumps,lung jumps', 'plank jacks, rope jump']
